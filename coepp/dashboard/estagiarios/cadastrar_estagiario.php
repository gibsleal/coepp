<?php
// dashboard/estagiarios/cadastrar_estagiario.php
// Formulário de cadastro de estagiário + UI de disponibilidade em grade (08:00–20:00).
// OBS: Posta em salvar_estagiario.php. Inclui máscaras, validações e presets de horário.
...
<div class="page-wrapper">
  <h2 class="page-title">Cadastro de Estagiário</h2>
  ...
  <style>
    ...
    .preset, .toggle-edit{
      height:30px; padding:0 10px; border-radius:8px; border:1px solid var(--border,#e5e7eb);
  ...
  <!-- OBS: POST vai para salvar_estagiario.php, que deve aplicar as mesmas regras de validação. -->
  <form method="POST" action="<?= url('dashboard/estagiarios/salvar_estagiario.php') ?>" autocomplete="off" id="form-estagiario">
    <div class="form-grid">
      ...
      <div class="form-group">
        ...
        <input class="input" type="text" id="supervisor" name="supervisor" required>
      </div>
      ...
      <div class="form-group full">
        ...
        <div class="availability">
          ...
          <?php
            // OBS: Dias úteis + sábado. Se quiser domingo, adicione onde necessário.
            ...
            <div class="day" data-day="<?= $dia ?>">
              <div class="day-head">
                ...
                <div class="presets">
                  ...
                  <button type="button" class="toggle-edit" onclick="toggleEditor('<?= $dia ?>')">Mostrar/ocultar</button>
                </div>
              </div>

              <!-- OBS: Começa aberta para facilitar seleção logo no primeiro cadastro. -->
              <div class="slots open" id="slots-<?= $dia ?>">
                <div class="slot-grid">
    ...
    <div class="form-actions">
      <a href="<?= url('dashboard/estagiarios/estagiarios.php') ?>"
         class="btn btn-secondary inline-center">
        Cancelar
      </a>
...
<script>
  ...
  // ---------- Presets e helpers de disponibilidade ----------
  ...
  function applyPreset(day,p){
    const boxes=getSlots(day);
    boxes.forEach(b=>b.checked=false);
    if(p==='none') return;
    const [s,e]=RANGES[p];
    boxes.forEach(b=>{const h=+b.dataset.hour; if(h>=s&&h<=e) b.checked=true;});
    document.getElementById('slots-'+day).classList.add('open');
  }
  ...
  // ---------- Validação extra (cliente): semestre 4–8 ----------
  document.getElementById('form-estagiario').addEventListener('submit', function(ev){
    const sem = parseInt(document.getElementById('semestre').value || '0', 10);
    if (!(sem >= 4 && sem <= 8)) {
      alert('Selecione o semestre entre 4º e 8º.');
      ev.preventDefault();
      return;
    }
  });
